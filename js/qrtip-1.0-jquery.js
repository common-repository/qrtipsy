...
(function($){
	$.fn.colorTip = function(settings){
		...
		*/
		...
		return this.each(function(){
			...
			/*$.get("http://ggl-shortener.appspot.com/?url="+encodeURIComponent(url)+"&jsonp=?",function(msg){

				// msg.query.results.item is an array:
				shortUrl = msg.short_url;
				//shortThis_uri = new URI(window.location.href);
				//shortUri = new URI(shortUrl);
				//shortUrl = shortUri.resolve(shortThis_uri);
				$('span.colorTip img').attr('src', 'http://chart.apis.google.com/chart?cht=qr&chs=' + settings.size + 'x' + settings.size + '&choe=UTF-8&chld=L%7C0&chl=' + shortUrl);
	...
	*/
	...
	/*$.fn.googl = function( options ) {  

		// Create some defaults, extending them with any options that were provided
		var settings = $.extend( {
		  'longUrl'         : 'http://goo.gl/CO9dO'
		}, options);
